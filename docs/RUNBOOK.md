...
**2. Put their URL in your private config** — `config/police/game.toml`:
...
(or `--role thief` if that is the role you agreed in A1)
...
| `Port 8802 already in use` | A previous peer is still alive | `Get-NetTCPConnection -LocalPort 8802 -State Listen` then `Stop-Process -Id <PID>` |
